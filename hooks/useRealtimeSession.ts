...
export function useRealtimeSession(sessionId: string | null) {
  ...
  const appendBattleLog = useCallback(
    async (message: string) => {
      try {
        ...
        if (!id) return
        await fetch(`/api/battles/${id}/log`, {
          method: "POST",
  ...
  useEffect(() => {
    ...
    const chIns = client
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "battles", filter: `session_id=eq.${sessionId}` },
        (payload) => {
          ...
          setSessionState((prev) => {
            ...
            return {
              ...
              chatLog: prev.chatLog.length ? prev.chatLog : (row.log || []).map(String),
            }
          })
        },
      )
    ...
    const chUpd = client
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "battles", filter: `session_id=eq.${sessionId}` },
        (payload) => {
          const row = payload.new as any
          setSessionState((prev) => {
  ...
  // Main load + realtime - NOW USES THE JOIN-SESSION API
  useEffect(() => {
    const client = clientRef.current
    if (!client) {
    ...
    const run = async () => {
      try {
        console.log("[useRealtimeSession] Attempting to join session via API:", sessionId)

        // Use the join-session API instead of direct Supabase query
        ...
        if (!joinResponse.ok) {
          ...
          console.error("[useRealtimeSession] Join session failed:", errorData)
        ...
        console.log("[useRealtimeSession] Successfully joined session:", joinData)

        const sessionData = joinData.session
        const mapData = sessionData.map

        // Process the map data
        const processedMap: MapData | null = mapData
          ? {
              ...mapData,
              tokens: (mapData.tokens || []).map((rt: any) => ({
            ...
            switch (event) {
              ...
              case "UPDATE_MAP": {
                ...
                const processed: MapData = {
                  ...incoming,
                  tokens: (incoming.tokens || []).map((rt: any) => ({
                    id: String(rt.id || uuidv4()),
                    type: rt.type === "monster" || rt.type === "pc" ? rt.type : "pc",
                    x: Number(rt.x || 0),
                    y: Number(rt.y || 0),
        ...
        newChannel.subscribe((status) => {
          if (status === "SUBSCRIBED") {
            console.log("[useRealtimeSession] Realtime channel subscribed successfully")
            setIsLoading(false)
          }
          if (status === "CHANNEL_ERROR") {
            console.error("[useRealtimeSession] Realtime channel error")
