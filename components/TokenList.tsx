...
function MiniRow({ t }: { t: MapToken }) {
  ...
  const row = (
    <div className="flex items-center justify-between text-xs py-1 px-2 rounded hover:bg-gray-700">
    ...
    <HoverCard openDelay={200}>
      ...
      <HoverCardContent className="bg-gray-800 text-gray-100 border-gray-700 max-w-sm">
        <div className="text-xs leading-relaxed space-y-2">
          ...
          <pre className="whitespace-pre-wrap break-words">{typeof notes === 'string' ? notes : JSON.stringify(notes, null, 2)}</pre>
        </div>
      </HoverCardContent>
    </HoverCard>
  )
...
export function TokenList({ monsters, pcs }: Props) {
  return (
    <div className="space-y-3">
      <div>
        ...
        <div className="rounded border border-gray-700 divide-y divide-gray-700">
          {monsters.length ? monsters.map((t) => <MiniRow key={t.id} t={t} />) : <div className="text-xs text-gray-500 p-2">No monsters</div>}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="rounded border border-gray-700 divide-y divide-gray-700">
          {pcs.length ? pcs.map((t) => <MiniRow key={t.id} t={t} />) : <div className="text-xs text-gray-500 p-2">No PCs</div>}
        </div>
      </div>
    </div>
  )
